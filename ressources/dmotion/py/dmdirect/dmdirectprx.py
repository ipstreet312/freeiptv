...
def snif(line):
    try:
        ...
        response = requests.get(url, proxies={proxy}, timeout=15).json()
        stream_url = response['qualities']['auto'][0]['url']
        m3u = requests.get(stream_url, proxies={proxy}, timeout=15).text
...
with open('ressources/dmotion/py/dmdirect/dmidprx.txt') as f:
